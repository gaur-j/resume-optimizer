...
/**
 * Contact information (name, phone, email, links) should never be
 * regenerated by an LLM. There's no upside to letting a model paraphrase
 * someone's own name or email, and in practice it can drop this block
 * entirely when the original resume's header isn't under a labeled
 * section (e.g. the name/contact line sits above the first
 * "PROFESSIONAL SUMMARY" heading with no heading of its own — exactly
 * what happened here). Instead we deterministically lift it from the
 * original resume text.
 *
 * Heuristic, not perfect: assumes line 1 of the resume is the candidate's
 * name (true for the vast majority of resumes) and collects the following
 * line(s) as contact info until something that looks like a section
 * heading (short, all-caps) shows up.
 */
function extractHeaderInfo(resumeText: string): {
  name: string;
  contactLines: string[];
} {
  ...
  for (let i = 1; i < lines.length && contactLines.length < 2; i++) {
    if (looksLikeHeading(lines[i])) break;
...
/**
 * Forces the tailored resume's first section to be the candidate's real
 * name + contact details, lifted verbatim from the original resume,
 * regardless of what the AI produced. Drops any section the AI generated
 * that was trying (and, per the bug report, sometimes failing) to be the
 * header, so it's never shown twice.
 */
function applyDeterministicHeader(
  result: TailoredResume,
  resumeText: string
): void {
  const { name, contactLines } = extractHeaderInfo(resumeText);

  const headerLines: TailoredResumeLine[] = [
    { type: "subheading", text: name, order: 0 },
  ...
  const nonHeaderSections = result.sections.filter(
    (section) =>
      !section.lines.some(
        (line) => line.text.trim().toLowerCase() === name.trim().toLowerCase()
      )
  );

  result.sections = [
    { heading: "", order: 0, lines: headerLines },
    ...nonHeaderSections.map((section, i) => ({ ...section, order: i + 1 })),
  ];

  result.full_text = renderFullText(result.sections);
}

/**
 * Whole-resume version of similarity() above. The old "no real changes"
 * guard only checked self-reported counters (bullets_rewritten,
 * keywords_added.length), which a model can satisfy with one trivial
 * edit while leaving the rest of the resume — summary, skills, anything
 * not typed as a "bullet" — completely untouched. This measures actual
 * word-overlap between the full original and full tailored text, so a
 * resume that's "technically changed" but still near-identical still
 * triggers the stronger retry. 0.95 is a starting threshold, not a
 * scientifically tuned one — adjust based on real output over time.
 */
function madeSubstantiveChanges(
  originalResumeText: string,
  tailoredFullText: string
): boolean {
  return similarity(originalResumeText, tailoredFullText) < 0.95;
}
...
3. Never fabricate employers, job titles, dates, degrees, certifications, projects, tools, skills, or metrics. If a target-role keyword names a tool, technology, or skill that nothing in the original resume evidences, do NOT add it to the summary or skills section - leaving a keyword out is always better than claiming a skill the candidate doesn't have.
...
11. Do not include the candidate's name or contact details as their own section - that's handled separately. Start your sections with the first real content section (e.g. Professional Summary).`;
...
- Also rewrite the professional summary and skills section wording (not just bullets) to better match the target role's language.
- If there are missing keywords listed below, naturally add at least 2-3 of them into the summary or skills section, but ONLY where truthful - skip any keyword that isn't evidenced anywhere in the original resume, even under this stronger instruction. It is better to add zero keywords than to fabricate one.
...
BULLET REWRITES TO APPLY (use preferred_rewrite for matching bullets):\n${bulletContext}`;
  ...
  if (result) {
    const madeNoRealChanges =
      (result.changes_summary.bullets_rewritten === 0 &&
        ...
        context.missingKeywords.length > 0) ||
      !madeSubstantiveChanges(resumeText, result.full_text);
  ...
  // Contact info is never left to the model — always force it in from the
  // original resume, on whichever attempt (first or retried) we end up with.
  applyDeterministicHeader(result, resumeText);
